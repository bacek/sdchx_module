...
Status OutputHandler::get_buf() {
  ...
  ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "sdchx get_buf");
  ...
  if (free_) {
    ...
    out_buf_ = ngx_create_temp_buf(r->pool, conf->buf_size);
    if (out_buf_ == NULL) {
      return STATUS_ERROR;
    }

    out_buf_->tag = (ngx_buf_tag_t) & sdchx_module;
    out_buf_->recycled = 1;
  }
